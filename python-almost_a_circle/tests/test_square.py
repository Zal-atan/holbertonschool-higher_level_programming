...
class TestBase(unittest.TestCase):
    ...
    def test_print_str(self):
        s1 = Square(3, 2, 3, 10)
        self.assertEqual(str(s1), "[Square] (10) 2/3 - 3/3")

    def test_update_function_args(self):
        s1 = Square(10, 10, 10, 10)
        self.assertEqual(str(s1), "[Square] (10) 10/10 - 10/10")
        s1.update(89)
        self.assertEqual(str(s1), "[Square] (89) 10/10 - 10/10")
        s1.update(89, 2)
        self.assertEqual(str(s1), "[Square] (89) 10/10 - 2/10")
        s1.update(89, 2, 3)
        self.assertEqual(str(s1), "[Square] (89) 10/10 - 2/3")
        s1.update(89, 2, 3, 4)
        self.assertEqual(str(s1), "[Square] (89) 4/10 - 2/3")
        s1.update(89, 2, 3, 4, 5)
        self.assertEqual(str(s1), "[Square] (89) 4/5 - 2/3")

    def test_update_function_kwargs(self):
        s1 = Square(10, 10, 10, 10)
        self.assertEqual(str(s1), "[Square] (10) 10/10 - 10/10")
        s1.update(height=1)
        self.assertEqual(str(s1), "[Square] (10) 10/10 - 10/1")
        s1.update(width=1, x=2)
        self.assertEqual(str(s1), "[Square] (10) 2/10 - 1/1")
        s1.update(y=1, width=2, x=3, id=89)
        self.assertEqual(str(s1), "[Square] (89) 3/1 - 2/1")
        s1.update(x=1, height=2, y=3, width=4)
        self.assertEqual(str(s1), "[Square] (89) 1/3 - 4/2")
